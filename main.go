...
const damage = "%s нанес урон противнику равный %d."
const Defence = "%s блокировал %d урона."
const unknownClass = "неизвестный класс персонажа"
...
func attack(charName, charClass string) string {
	if charClass == "warrior" {
		return fmt.Sprintf(damage, charName, 5+Rand(3, 5))
	}

	if charClass == "mage" {
		return fmt.Sprintf(damage, charName, 5+Rand(5, 10))
	}

	if charClass == "healer" {
		return fmt.Sprintf(damage, charName, 5+Rand(-3, -1))
	}
	return unknownClass
}
...
func defence(charName, charClass string) string {
	if charClass == "warrior" {
		return fmt.Sprintf(Defence, charName, 10+Rand(5, 10))
	}
	if charClass == "mage" {
		return fmt.Sprintf(Defence, charName, 10+Rand(-2, 2))
	}
	if charClass == "healer" {
		return fmt.Sprintf(Defence, charName, 10+Rand(2, 5))
	}
	return unknownClass
}
